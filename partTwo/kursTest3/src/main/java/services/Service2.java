...
public class Service2
{
  ...
  public static void printResultsPostEndpoint(@RequestBody String results) throws IOException {
    String decodedJson = java.net.URLDecoder.decode(results, "UTF-8");
    ObjectMapper jacksonObjectMapper = new ObjectMapper(); // This is Jackson
    ArrayList<Service1.Ship> stats =  jacksonObjectMapper.readValue(
            decodedJson, new TypeReference<ArrayList<Service1.Ship>>(){});
  ...
  public static void createJsonSchedule(@RequestParam(value = "myName", defaultValue = "schedule")
                                                   String name, ObjectMapper mapper, String schedule) throws IOException {
    ...
    if (schedule.isEmpty()){
      RestTemplateBuilder builder = new RestTemplateBuilder();
      RestTemplate restTemplate = builder.build();
      schedule = restTemplate.getForObject("http://localhost:8080/scheduleGenerate", String.class);
    }
    mapper.writeValue(g, schedule);
  }
